...
flx_all_ports() {
    find "$FREELINX_ROOT/base" "$FREELINX_ROOT/shells" "$FREELINX_ROOT/net" "$FREELINX_ROOT/firmware" "$FREELINX_ROOT/sysutils" -mindepth 1 -maxdepth 1 -type d 2>/dev/null \
        | sed 's#^.*/##' | sort -u
}
...
for _p in $(flx_all_ports); do
    _d=""
    _catname=""
    for _cat in base shells net firmware sysutils; do
